...
const SecondSlider = ({locale}) => {
    const {t} = useTranslation();
    if(!locale) locale = localStorage.getItem("i18nextLng") || "ru"
    return (
        <div className={"first__slide__wrap second__slider"}>
            <div className={"first__slide__container"}>
                <h1 className="first__slide__title second__slider">{t("mainPage.slider.title")}<span>тм</span></h1>
                <div className="first__slide__description">{t("mainPage.slider.secondDescription")}</div>
